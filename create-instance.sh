...
if [ $? -eq 0 ]; then
   echo "Instances $INSTANCE_NAME is already running"
   exit 0
fi    
...
if [ $? -eq 0 ]; then
   echo "Instances $INSTANCE_NAME is already created and stopped"
   exit 0
fi    
...
aws route53 change-resource-record-sets --hosted-zone-id Z045299827N9EFZA0BWFI --change-batch file:///tmp/record.json | jq